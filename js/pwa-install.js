(function () {
  ...
  if (localStorage.getItem('tgi_pwa') === oggi) return;

  // Stili
  const s = document.createElement('style');
  s.textContent = `
    @keyframes tgiUp{from{transform:translateY(100%);opacity:0}to{transform:translateY(0);opacity:1}}
    ...
    #tgi-banner{
      ...
      animation:tgiUp .35s ease;
    }
    #tgi-banner .gb-ico{width:52px;height:52px;border-radius:13px;background:#0d0d0d;border:1px solid #333;overflow:hidden;flex-shrink:0}
    #tgi-banner .gb-ico img{width:100%;height:100%;object-fit:cover;display:block}
  ...
  if (isAndroid) {
    ...
    banner.innerHTML = `
      ...
      <div class="gb-txt">
        <div class="gb-t1">Installa T.G.I COMO</div>
    ...
    document.getElementById('gb-ok').addEventListener('click', async () => {
      banner.remove();
      localStorage.setItem('tgi_pwa', oggi);
      if (deferred) {
        deferred.prompt();
    ...
    document.getElementById('gb-ok').addEventListener('click', () => {
      ...
      g.innerHTML = `
        <div class="gi-hd">
          ...
          <button onclick="this.closest('#tgi-ios').remove()">✕</button>
        </div>
